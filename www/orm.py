...
def select(sql, args, size = None):
    ...
    with (yield from __pool) as conn:
        ...
        # 设置执行语句，其中sql语句的占位符为？，而python为%s, 这里要做一下替换,args是sql语句的参数
...
class Model(dict, metaclass = ModelMetaclass):
    ...
    def getValueOrDefault(self, key):
        ...
        if value is None:
            ...
            if field.default is not None:
                ...
                logging.debug("using defaullt value for %s: %s" % (key, str(value)))
                # 把默认值设为这个属性的值
    ...
    @asyncio.coroutine
    def save(self):
        args = list(map(self.getValueOrDefault, self.__fields__))  # 将除主键外的所有属性名添加到args这个列表中
    ...
    @asyncio.coroutine
    def update(self):
        args = list(map(self.getValue, self.__fields__))    # 将参数中提供除主键外的属性名添加到args这个列表中
        args.append(self.getValue(self.__primary_key__))    # 再把主键添加到这个列表的最后
