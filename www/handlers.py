...
# _RE_EMAIL是Email的字符串匹配格式，_RE_SHA1密码字符串匹配格式
...
def text2html(text):
    lines = map(lambda s: '<p>%s</p>' % s.replace('&', '&amp;').replace('<','&it;')\
        .replace('>','&gt;'), filter(lambda s: s.strip() != '', text.split('\n')))
    return ''.join(lines)

# cookie:
def user2cookie(user, max_age):
    # 通过id-expires-sha1来建立cookie
...
def authenticate(*, email, passwd):
    ...
    if not passwd:  
        raise APIValueError('passwd', 'Inavalid password')
...
def api_register_user(*, email, name, passwd):
    if not name or not name.strip():            # strip()去除字符串两端的空格
...
def api_create_blog(request, *, name, summary, content):
    ...
    blog = Blog(user_id = request.__user__.id, user_name = request.__user__.name, user_image = \
        request.__user__.image, name = name.strip(), summary = summary.strip(), content = content.strip())
