...
async def response_factory(app, handler):
    async def response(request):
        ...
        return resp
    #上面6个if其实只用到了一个，准确的说只用到了半个。
    #大家可以把用到的代码找出来，把没有用到的注释掉，如果程序能正常运行，那我觉得任务也就完成了
...
# 定义首页
def index(request):
    return web.Response(body=b"<h1>Justin-Han</h1>", content_type="text/html", charset="UTF-8")
# 调用asyncio实现异步IO
async def init(loop):
    # 创建数据库连接池
    await orm.create_pool(loop=loop, host='127.0.0.1', port=3306, user='root', password='12125772', db='awesome')
